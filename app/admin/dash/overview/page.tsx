...
function RecentQueries() {
  ...
  // Since we're using app router, we need to adjust how we check the current path
    ...
    debounceTimeout.current = setTimeout(async () => {
      try {
        if (activeTab === 'ask') {
          ...
          setAskResponse(null);
          const searchPromise = fetch(`/api/search?query=${encodeURIComponent(searchQuery)}`).then(res => res.json());
                    ...
                    onMouseDown={async () => {
                      ...
                      try {
                        if (activeTab === 'ask') {
                          ...
                          // Find mode: call both search and synapse in parallel
                          setAskResponse(null);
                          const searchPromise = fetch(`/api/search?query=${encodeURIComponent(query)}`).then(res => res.json());
                      ...
                      {(() => {
                        if (askResponse && askResponse.answer) {
                          ...
                          if (askResponse.details) {
                            ...
                            } else if (details.details_type === 'bullet points' && typeof details.content === 'string') {
                              detailsContent = (
                                <ReactMarkdown components={customRenderers}>
                                  {details.content}
                                </ReactMarkdown>
